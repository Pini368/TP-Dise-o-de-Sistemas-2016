    ...
    public class PuestoDB : PuestoDAO
    {
        ...
        public List<Puesto> getPuestos(Expression<Func<Puesto, bool>> filtro)
        {
            using (TPDiseñoEntities db = new TPDiseñoEntities())
            {
                ...
                if(filtro != null)
                {
                    return db.Puesto.Include("Puntaje_Requerido.Competencia").Include("Evaluacion.Cuestionario").Include("Evaluacion.Cuestionario.Estado_Cuestionario").Where(filtro).Where(predicadoGeneral).ToList();
                }
                else
                {
